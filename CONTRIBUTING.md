...
For a release-shaped local check, also run `make dmg VERSION=2.0.0 CHANNEL= BUILD=1` and
...
`(cd dist && shasum -a 256 -c ScreenCap-2.0.0.dmg.sha256)`. The CI workflow is the canonical
